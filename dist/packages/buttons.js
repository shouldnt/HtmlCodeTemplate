...
!(function() {
		...
		F = function(e, t) {
			...
			if (!(o.push(t) > 1)) {
				...
				if (i) {
					...
					var c = g(l.document)('script', { async: !0, src: e + (/\?/.test(e) ? '&' : '?') + 'callback=_' }),
						s = function() {
							l._ && l._({ meta: {} });
						};
					d(c, 'load', s),
						d(c, 'error', s),
						c.readyState && f(c, /de|m/, s),
						l.document.getElementsByTagName('head')[0].appendChild(c);
				}
			}
		},
		k = function(e, t, o) {
			var r = g(e.ownerDocument),
				n = e.appendChild(r('style', { type: 'text/css' })),
				a =
					...
					(function(e) {
						...
						return (
							m[b(m, t['no-preference']) ? t['no-preference'] : 'light'] +
							v('light', t.light) +
							v('dark', t.dark)
						);
					})(t['data-color-scheme']);
			...
			var i,
				...
				d = r(
					'a',
					{
						...
						target: '_blank',
						rel: 'noopener',
						innerHTML: ((i = t['data-icon']),
						(l = /^large$/i.test(t['data-size']) ? 16 : 14),
						(i = ('' + i).toLowerCase().replace(/^octicon-/, '')),
						b(w, i) || (i = 'mark-github'),
						'<svg version="1.1" width="' +
							l * w[i].width / w[i].height +
							'" height="' +
							l +
							'" viewBox="0 0 ' +
							w[i].width +
			...
			/\.github\.com$/.test('.' + d.hostname)
				? /^https?:\/\/((gist\.)?github\.com\/[^/?#]+\/[^/?#]+\/archive\/|github\.com\/[^/?#]+\/[^/?#]+\/releases\/download\/|codeload\.github\.com\/)/.test(
						d.href
					) && (d.target = '_top')
				: ((d.href = '#'), (d.target = '_self'));
			var s,
				u,
				f,
				h,
				p = e.appendChild(
					r('div', { className: 'widget' + (/^large$/i.test(t['data-size']) ? ' widget-lg' : '') }, [ d ])
				);
			/^(true|1)$/i.test(t['data-show-count']) &&
			'github.com' === d.hostname &&
			(s = d.pathname
				.replace(/^(?!\/)/, '/')
				.match(/^\/([^/?#]+)(?:\/([^/?#]+)(?:\/(?:(subscription)|(fork)|(issues)|([^/?#]+)))?)?(?:[/?#]|$)/)) &&
			!s[6]
				? (s[2]
						? ((u = '/repos/' + s[1] + '/' + s[2]),
							s[3]
								? ((h = 'subscribers_count'), (f = 'watchers'))
								: s[4]
									? ((h = 'forks_count'), (f = 'network'))
									: s[5]
										? ((h = 'open_issues_count'), (f = 'issues'))
										: ((h = 'stargazers_count'), (f = 'stargazers')))
						: ((u = '/users/' + s[1]), (f = h = 'followers')),
					F.call(this, 'https://api.github.com' + u, function(e, t) {
						if (!e) {
							var n = t[h];
							p.appendChild(
								r(
									'a',
									{
										...
										'aria-label':
											n +
											' ' +
											h.replace(/_count$/, '').replace('_', ' ').slice(0, n < 2 ? -1 : void 0) +
											' on GitHub'
									},
			...
			if (null != t && null != r)
				if (
					(t.getAttribute &&
						(t = (function(e) {
							for (
								var t = { href: e.href, title: e.title, 'aria-label': e.getAttribute('aria-label') },
									o = [ 'icon', 'color-scheme', 'text', 'size', 'show-count' ],
									r = 0,
									n = o.length;
								r < n;
								r++
					...
					var n = h('span', { title: t.title || void 0 });
	...
	t.protocol + '//' + t.host + t.pathname === a
		? k(e.body, c(window.name || t.hash.replace(/^#/, '')))
		: (function(t) {
				...
				for (
					var t = e.querySelectorAll
							? e.querySelectorAll('a.github-button')
							: (function() {
									for (var t = [], o = e.getElementsByTagName('a'), r = 0, n = o.length; r < n; r++)
										~(' ' + o[r].className + ' ')
											.replace(/[ \t\n\f\r]+/g, ' ')
											.indexOf(' github-button ') && t.push(o[r]);
									return t;
								})(),
						o = 0,
						r = t.length;
